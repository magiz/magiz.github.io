...
class Torrent {
	...
	*/
	...
	static protected function decode ( $string ) {
		$data = /*is_file( $string ) || Commented by ziphir */ self::url_exists( $string ) ?
	...
	protected function build ( $data, $piece_length ) {
		...
		//elseif ( is_dir( $data ) ) commented by ziphir
	...
	static public function format ( $size, $precision = 2 ) {
		$units = array ('<span>bytes</span>', '<span>KB</span>', '<span>MB</span>', '<span>GB</span>', '<span>TB</span>');
